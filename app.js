...
function dailyPuzzle(seedStr) {
  const rng = mulberry32(hashString(seedStr));
  const idx = ABODES_PUZZLES.map((_, i) => i);
...
// --- Storage ----------------------------------------------------------------
const storageKey = () => `abodes:${dateKey()}`;
function loadResult() {
...
const progressKey = () => `abodes:progress:${dateKey()}`;
function saveProgress(cells) {
  try { localStorage.setItem(progressKey(), JSON.stringify(cells)); } catch {}
}
...
function startGame() {
  ...
  const puzzle = dailyPuzzle("abodes");
  const n = puzzle.size;
...
function finishGame() {
  ...
  const result = { date: dateKey(), seconds: Math.round(sec) };
  saveResult(result);
  clearProgress();
  bumpStreak();
  // Best-effort leaderboard submission (no-op until the leaderboard is built).
...
function renderResults(result, replay) {
  $("#results-title").textContent = replay ? "Already solved today" : "Solved!";
  ...
  buildGrid($("#solved-board"), puzzle, solvedCells, /*interactive*/ false);

  showView("results");
}

// Spoiler-free share text: date + time + streak. Never the layout.
function buildShareText() {
  const res = loadResult();
  if (!res) return `Abodes — ${dateKey()}`;
  const streak = currentStreak();
  const tail = streak >= 2 ? `  🔥 ${streak}` : "";
  return `Abodes — ${res.date}\n⛺ Solved in ${fmtElapsed(res.seconds)}${tail}`;
}
...
function refreshMenu() {
  ...
  $("#status-daily").textContent = res
    ...
    : (loadProgress() ? "In progress…" : "Not solved today");
  $("#menu-share-btn").hidden = !res;

  // Mini preview art on the menu card.
  $("#menu-art").innerHTML = `${treeSVG()}${tentSVG()}${treeSVG()}`;
}

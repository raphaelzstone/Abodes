...
N = 8                       # 8x8 grid
TENT_MIN, TENT_MAX = 10, 14 # tents (== trees) per board; medium density for 8x8
POOL_SIZE = 1500            # how many distinct puzzles to emit (~4 years of dailies)
...
def main():
    ...
    header = (
        ...
        " * Each puzzle is an 8x8 Tents board verified to be solvable by pure\n"
        " * deduction (no guessing) and to have a unique solution.\n"
        " * Fields: size, trees [[r,c]...], tents (solution), rowClues, colClues. */\n"
    )
    body = "const ABODES_PUZZLES = " + json.dumps(puzzles, separators=(",", ":")) + ";\n"
    with open("puzzles.js", "w") as f:
        f.write(header + body)
    print(f"Wrote {len(puzzles)} puzzles to puzzles.js ({attempts} attempts).",
          file=sys.stderr)
